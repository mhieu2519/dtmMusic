...
async function handlePlay(interaction, player) {
  ...
  await interaction.deferReply();
    ...
    if (!musicChannelId) {
        return interaction.editReply({ content: 'Lỗi cấu hình: Không tìm thấy MUSIC_CHANNEL_ID trong .env!', flags: MessageFlags.Ephemeral});
    }
    ...
    if (!targetChannel || targetChannel.type !== 2) { // 2 là Discord.ChannelType.Voice
        return interaction.editReply({ content: `Kênh thoại với ID "${musicChannelId}" không hợp lệ hoặc không tồn tại. Vui lòng kiểm tra lại MUSIC_CHANNEL_ID trong file .env!`, flags: MessageFlags.Ephemeral});
    }
    ...
    if (!permissions.has('Connect') || !permissions.has('Speak')) {
        return interaction.editReply({ content: `Lão phu không có quyền **Kết nối** hoặc **Nói chuyện** trong kênh thoại ${targetChannel.name}.` });
    }
